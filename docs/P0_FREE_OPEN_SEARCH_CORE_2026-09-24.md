...
4. **Ranking P0**: se conserva la relevancia existente y se agregan señales de confianza de dominio, frescura y penalización básica de spam. Se implementó BM25 sobre el índice local limitado de metadatos y páginas permitidas (500 entradas volátiles); NO equivale al ranking de un índice global. Continúan diversidad de dominio, filtros site:/after:/before:/exclusiones/frases y priorización de sitios nombrados.
...
| Ranking | AVANZADO P0 | Relevancia léxica, BM25 sobre índice WAE local efímero, señales de dominio/frescura/spam/diversidad. BM25 global y reranker por embeddings aún requieren índice persistente. |
