...
test("general web integrates Wikipedia honestly and explicit wiki lookup stays available", async () => {
  ...
  try{
    ...
    assert.ok(visited.some(url=>url.includes("wikipedia.org")));
    assert.ok(visited.some(url=>url.includes("wikidata.org")));
