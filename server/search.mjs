...
export async function quickOpenWeb(query){
  ...
  if(!technical&&!repoTerms&&!crateTerms&&previous.length>=3){
  ...
  const sourceCounts=new Map(),limit=technical||repoTerms||crateTerms?8:4;
  const hits=ranked.filter(item=>{
    const count=sourceCounts.get(item.source)||0;
    if(count>=2&&(technical||repoTerms||crateTerms))return false;
  ...
  const payload = {
    ...
    searchCoverage:selected==="all"?{
      ...
      specialistSources:["WAE WEB · directorio","Wikidata · sitios web","WAE Discovery","WAE Index local","Stack Overflow",
        "Super User","MDN Web Docs","GitHub · repositorios públicos",CRATES_SOURCE,"Wikipedia","Wikidata"]
