...
import {searchPoi,PoiError} from "./poi.mjs";
...
const files = new Map([
  ...
  ["/libros", ["libros.html", "text/html; charset=utf-8"]],
  ["/libros.html", ["libros.html", "text/html; charset=utf-8"]],
  ["/libros.js", ["libros.js", "text/javascript; charset=utf-8"]],
...
function translationLimited(req){
  const ip=process.env.TRUST_PROXY==="true"
    ?(req.headers["x-forwarded-for"]||"").split(",")[0].trim()||req.socket.remoteAddress
    :req.socket.remoteAddress||"unknown";
  const now=Date.now(),record=translationRate.get(ip);
  if(!record||now>record.expires){
    if(translationRate.size>10000)translationRate.clear();
    translationRate.set(ip,{count:1,expires:now+60000});return false;
  }
  record.count++;return record.count>8;
}
...
export async function handler(req, res) {
      ...
      (!(["GET","HEAD"].includes(req.method) ||
        ...
        !([ "/api/translate","/api/directions" ].includes(u.pathname) ||
          ["/api/health","/api/capabilities","/api/search",
           "/api/weather","/api/maps","/api/places","/api/poi","/api/marketplace",
  ...
  if (u.pathname === "/api/capabilities") return write(res, 200, {
    ...
    poiSearch:{source:"OpenStreetMap Overpass public",scope:"on_demand_bounded_local",
      cost:"no_api_billing",coverage:"OSM tagged businesses within selected exploratory area",
      limitPerInstancePerDay:120,notGuaranteed:true},
  ...
  if (u.pathname.startsWith("/api/")) {
    ...
    try {
      ...
      if(u.pathname==="/api/poi"){
        if(!["GET","HEAD"].includes(req.method))return write(res,405,{error:"Método no permitido."},{allow:"GET, HEAD"});
        if(directionsLimited(req))return write(res,429,{error:"Demasiadas búsquedas locales.",code:"poi_rate_limit"},{"retry-after":"60"});
        return write(res,200,await searchPoi(u.searchParams.get("q")||""));
      }
