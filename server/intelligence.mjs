...
export function parseQuery(raw) {
  ...
  const sourceMatch = input.match(/(?:^|\s)source:(wikipedia|crossref|openalex|google|wikimedia)(?=\s|$)/i);
  ...
  const query = input
    ...
    .replace(/(?:^|\s)source:(?:wikipedia|crossref|openalex|google|wikimedia)(?=\s|$)/gi, " ")
